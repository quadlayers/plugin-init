...
fs.rm( pluginFolder, { recursive: true }, ( err ) => {
	...
	fs.mkdir( pluginFolder, { recursive: true }, ( err ) => {
		...
		if ( myArgs.includes( '--wc' ) ) {
			...
			const pluginFileMainContent = fs.readFileSync(
				pluginFileMain,
				'utf-8'
			);
			// Replace lines to white space
			const newContent = pluginFileMainContent.replace(
				"require_once __DIR__ . '/vendor_packages/wp-notice-plugin-required.php';",
				''
			);
			const pluginFileMainContentEdited = newContent.replace(
				"require_once __DIR__ . '/vendor_packages/wp-license-client.php';",
				''
			);
			fs.writeFileSync(
				pluginFileMain,
				pluginFileMainContentEdited,
				'utf-8'
			);
		}
		/**
